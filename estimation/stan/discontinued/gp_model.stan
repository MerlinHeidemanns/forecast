...
data {
  ...
  real trend_long_term_length_scale_data;
...
transformed data {
  ...
  matrix[n_layer3_padding, n_parties - 1] trend_long_term_zero_matrix  = rep_matrix(0.0, n_layer3_padding, n_parties - 1);
  ...
  // Covariance long term trend
  vector[n_layer3_padding %/% 2 + 1] trend_long_term_cov_rfft = gp_periodic_exp_quad_cov_rfft(
    n_layer3_padding, // Scale
    1, // sigma
    trend_long_term_length_scale_data, // Length scale
    n_layer3_padding) + 1e-9; // Periodicity
}

parameters {
  // GP and correlation parameters
  matrix[n_layer1_padding, n_parties - 1] trend_short_term_std_normal;
  matrix[n_layer3_padding, n_parties - 1] trend_long_term_std_normal;
  ...
  real<lower = 0> volatility_long_term_scale;
...
transformed parameters {
  ...
  matrix[n_layer3, n_parties] trend_long_term;
  ...
  profile("rfft_matrix_inversion") {
    ...
    // -- Compute f
    trend_long_term[, 2:n_parties] = volatility_long_term_scale *             // Long-term volatility
                                      gp_inv_rfft_multi_outcome(         // GP
                                        trend_long_term_std_normal, 
                                        trend_long_term_zero_matrix, 
                                        trend_long_term_cov_rfft
                                      )[1:n_layer3] * 
                                      diag_pre_multiply(alpha, L_Omega)';
    trend_long_term[, 1] = rep_vector(0.0, n_layer3);

    
    // Add mean
    trend[, 1] = rep_vector(0.0, n_layer1);
    trend[, 2:n_parties] = trend_short_term[, 2:n_parties] + 
                           trend_long_term[trend_layer3_idx, 2:n_parties] +
...
model {
  ...
  volatility_long_term_scale ~ normal(0, 0.2);
  ...
  to_vector(trend_long_term_std_normal) ~ std_normal();
...
generated quantities {
  ...
  // Compute long-term trend
  {
    matrix[n_layer1, n_parties] tmp;
    tmp[, 1] = rep_vector(0.0, n_layer1);
    tmp[,2:n_parties] = trend_long_term[trend_layer3_idx, 2:n_parties] +
                           rep_vector(1.0, n_layer1) * trend_mean;
    trend_long_term_shares = exp(tmp - log(exp(tmp) * ones));
  }
