...
data {
  ...
  // Election data
  real time_aggregation;
  real polling_error_correlation;
  matrix[n_elections, n_parties_fixed + n_parties_trans] election_y;
...
transformed data {
  ...
  vector[n_layer1_padding %/% 2 + 1] polling_error_cov_rfft = 
    gp_periodic_exp_quad_cov_rfft(
      n_layer1_padding, // Scale
      1, // sigma
      - (365.0 * 4 / time_aggregation) / log(polling_error_correlation),  // Length scale
      n_layer1_padding) + 1e-9; // Periodicity
}
...
parameters {
  ...
  // Polling error
  real<lower = 0> polling_error_scale;
  matrix[n_layer1_padding, n_parties_fixed + n_parties_trans - 1] polling_error_std_normal;
...
transformed parameters {
  ...
  matrix[n_elections, n_parties_fixed + n_parties_trans] election_trend_probabilities;
  
  // Adjustment terms
  matrix[n_surveys, n_parties_fixed + n_parties_trans] rounding_error;
  matrix[n_layer1, n_parties_fixed + n_parties_trans] polling_error;
  //
  //
  profile ("volatility_computation"){
  ...
  polling_error[,2:n_parties_fixed + n_parties_trans] = polling_error_scale * gp_inv_rfft_multi_outcome(
                                        polling_error_std_normal, 
                                        trend_short_term_zero_matrix, 
                                        polling_error_cov_rfft
                                      )[1:n_layer1] * 
                                      diag_pre_multiply(alpha, trend_party_correlation_chol)';
  polling_error[, 1] = rep_vector(0.0, n_layer1);
  
  profile("softmax") {
    matrix[n_surveys, n_parties_fixed + n_parties_trans] surveys_tmp = trend[survey_layer1_idx,] + polling_error[survey_layer1_idx,];
    matrix[n_elections, n_parties_fixed + n_parties_trans] elections_tmp = trend[election_layer1_idx,];
...
model {
  ...
  //target += -1e4 * sum(columns_dot_product(election_y, election_trend_probabilities));
  target += -1e3 * sum(square(election_y[1:2,] - election_trend_probabilities[1:2,]));
}
