...
transformed data {
  ...
  int layer1_padding = 150;
  int layer2_padding = 50;
  int n_layer1_padding = n_layer1 + layer1_padding;
  int n_layer2_padding = n_layer2 + layer2_padding;
...
parameters {
  ...
  vector[n_layer1_padding * (n_parties_fixed + n_parties_trans - 1)] trend_short_term_std_normal;
...
transformed parameters {
...

  {
    ...
    profile("rfft_matrix_inversion_short_term"){
      for (i in 1:n_parties_fixed){
        {
        int start_idx = 1 + n_layer1_padding * (i - 1);
        int end_idx = n_layer1_padding * i;
        trend[ ,i] = volatility_short_term .* gp_inv_rfft(
                                        trend_short_term_std_normal[start_idx:end_idx], 
                                        trend_short_term_zero_matrix, 
                                        trend_cov_rfft
                                      )[1:n_layer1];
        }
      }
      for (i in 1:n_parties_trains){
        {
          int n_duration = n_party_trans_duration[i] + layer1_padding;
          int start_idx  = 1 + n_layer1_padding * n_parties_fixed + ;
          int end_idx;
          vector[n_duration %/% 2 + 1] trend_cov_rfft_trans = gp_sum_exp_quad_cov_rfft(
                                                n_duration, 
                                                1, 
                                                1, 
                                                trend_length_scale[1],
                                                trend_length_scale[2], 
                                                n_duration) + 1e-9;;
          trend_cov_rfft = volatility_short_term .* gp_inv_rfft(
                                        trend_short_term_std_normal[start_idx:end_idx], 
                                        trend_short_term_zero_matrix, 
                                        trend_cov_rfft_trans
                                      )[1:n_layer1];
        }
      }
    }
