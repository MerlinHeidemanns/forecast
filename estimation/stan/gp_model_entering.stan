...
data {
  ...
  array[n_parties] int<lower=1, upper = n_surveys> n_parties_presence;         // in how many surveys was the party asked about
  array[n_parties, n_surveys] int parties_survey_idx;       // which surveys ask for party i
  array[n_surveys] int<lower=1,upper=R> reporting_pattern;  // which pattern each survey follows
  array[n_surveys, n_parties_trans] int<lower=0,upper=1> pattern_matrix; // which trans parties are reported in each pattern
  ...
  array[n_surveys, n_parties + n_parties_trans] int survey_y;
...
parameters {
  ...
  matrix[n_layer1_padding, n_parties + n_parties_trans - 1] trend_short_term_std_normal;
  cholesky_factor_corr[n_parties + n_parties_trans - 1] trend_party_correlation_chol;
  positive_ordered[2] trend_length_scale;

  // Alpha parameters
  vector[n_parties + n_parties_trans - 2] z_alpha;
  ...
  // Long term trend
  matrix[n_knots, n_parties + n_parties_trans - 1] spline_coefficients;    // spline coefficients for P-1 parties (relative to reference)

  // Transition rates
  array[n_parties_trans] transition_rate;
  
  // Rounding error
  matrix<lower = -1, upper = 1>[n_surveys, n_parties + n_parties_trans] rounding_error_std_normal;
...
transformed parameters {
  ...
  matrix[n_layer1, n_parties + n_parties_trans] trend_short_term;
  matrix[n_layer1, n_parties + n_parties_trans] trend_long_term;
  matrix[n_layer1, n_parties + n_parties_trans] trend;
  // predictions
  matrix[n_surveys, n_parties + n_parties_trans] survey_trend_probabilities;
  matrix[n_elections - 1, n_parties + n_parties_trans] election_trend_probabilities;
  matrix[n_surveys, n_parties + n_parties_trans] rounding_error;
  matrix[n_elections, n_parties + n_parties_trans] polling_error;
  //
  //
  ...
  profile("rfft_matrix_inversion") {
    ...
    alpha[2:n_parties + n_parties_trans - 1] = exp(z_alpha);
    alpha[1] = 1;
    ...
    trend_long_term[ ,2:n_parties + n_parties_trans] = spline_basis * spline_coefficients;
    ...
    for (i in 1:n_parties_trans) {
      vector[n_layer1] w = party_weight(vec_n_layer1_lin, t_emerge[i], transition_rate[i]);
      f[,n_parties_fixed + i] = w .* trend[,n_parties_fixed + i] + (1 - w) * (negative_logit_zero);
    }
  }
...
model {
  ...
  if (flag_inference == 1) {
    profile("llh_contributions") {
      for (i in 1:n_parties) {
        survey_y[parties_survey_idx[1:n_parties_presence[k]], i] ~ 
          poisson((survey_trend_probabilities[parties_survey_idx[1:n_parties_presence[k]], i] + 
                               rounding_error[parties_survey_idx[1:n_parties_presence[k]], i]) .* to_vector(n_survey_responses)[parties_survey_idx[1:n_parties_presence[k]]]);
      }
    }
  }
