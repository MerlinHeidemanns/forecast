...
data {
  ...
  // Vote counts and observation indices
  array[K_fixed + K_trans] int<lower=0> N_obs;  // number of observations per party
  array[K_fixed + K_trans, N] int y_obs;        // vote counts for each party
  array[K_fixed + K_trans, N] int<lower=0,upper=N> obs_idx;  // observation indices for each party
  
  // Survey reporting structure
  int<lower=1> R;                     // number of different reporting patterns
  array[R, 2] int pattern_starts;
...
transformed data {
  matrix[K_fixed + K_trans, K_fixed + K_trans] ones = rep_matrix(1.0, K_fixed + K_trans, K_fixed + K_trans);
...
parameters {
  ...
  matrix[T, K_fixed + K_trans - 1] eta;  // GP random effects
...
transformed parameters {
  ...
  matrix[N, K_fixed + K_trans] probs; // base probability matrix
  ...
  f[,1:(K_fixed + K_trans - 1)] = L_k * eta * L_Omega';
  f[,K_fixed + K_trans] = rep_vector(0.0, T);  // reference category
  ...
  // Calculate base probabilities
  {
    ...
    probs = exp(tmp - log(exp(tmp) * ones));  // Manual softmax is faster
  }
  
  // Calculate probabilities under each reporting pattern using matrix multiplication
  for (r in 1:R) {
    int start = pattern_starts[r, 1];
    int end = pattern_starts[r, 2];
...
model {
  ...
  alpha ~ normal(0, 2);
  rho ~ inv_gamma(5, 5);
  transition_rate ~ normal(0, 10);
  L_Omega ~ lkj_corr_cholesky(2);
  to_vector(eta) ~ std_normal();
  
  // Likelihood
  for (k in 1:(K_fixed + K_trans)) {
    y_obs[k, obs_idx[k,1:N_obs[k]]] ~ poisson(n_resp[obs_idx[k,1:N_obs[k]]] .* 
                                              pattern_probs[obs_idx[k,1:N_obs[k]], k]);
  }
}
...
generated quantities {
  ...
  matrix[T, K_fixed + K_trans] shares;
  ...
  {
    matrix[T, K_fixed + K_trans] tmp = f;
    shares = exp(tmp - log(exp(tmp) * ones));  // Manual softmax is faster
  }
